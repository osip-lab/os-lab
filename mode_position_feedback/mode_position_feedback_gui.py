...
import pyperclip
import numpy as np
from piezo_feedback_gui import PiezoFeedbackWidget
...
class BaslerCamControlWidget(ThreadedWidget):
    ...
    def __init__(self, font_size=14):
        ...
        self.settings = {'exposure': 2500, 'gain': 0.0}
...
class GaussianFitterWorker(ThreadedWorker):
    ...
    def fit(self, data):
        fit, pars = fit_gaussian(data['image'], rebinning=8)
        data['gaussian'] = fit
...
class MainWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.locker = PiezoFeedbackWidget(font_size=self.font_size)
        ...
        self.fitter.sig_fitted.connect(self.locker.feedback)
        self.plotter.sig_plotted.connect(self.controller.plotter_command)

        layout = QMyVBoxLayout(self.controller, self.fitter, self.locker, self.plotter, alignment=Qt.AlignmentFlag.AlignCenter)
        self.setLayout(layout)
