...
Pipeline
--------
1. Load a PicoScope CSV (file chosen via the clipboard).
...
SIGNAL_COLUMN = 'Channel D'       # intensity column to analyse
...
# cavity / spectrum / dependency plots are informative but each blocks.
...
# %% [Step 1] Load the PicoScope CSV ----------------------------------------
csv_path = wait_for_path_from_clipboard(filetype='csv')
...
if analysis_mode == 'fit':
    ...
    ax.set_title('Fit result (close window to continue)')
    ...
    ax.set_title('Point-selection mode - no fit (close window to continue)')
